...
RC deQueue()
{
	...
	RC curpage = 0;
	while (curpage < (*q).totalNumOfFrames)
	{
...
RC FIFO(BM_BufferPool *const bm, BM_PageHandle *const page, const PageNumber pageNum)
{
	...
	// Initializing the pageInfo variables :-
	pageInfo *list = NULL;
	pageInfo *temp = NULL;
	pageInfo *temp1 = NULL;
	// Setting the list pointer
	list = (*q).head_ptr;
	RC curpage = 0;
...
{
	 // Check if the file exists before initializing the buffer pool
    if (access(pageFileName, RC_OK) != 0) {
...
RC shutdownBufferPool(BM_BufferPool *const bm) {
    ...
    if (!fh) {
        return RC_BUFFER_POOL_NOT_OPEN;
    }
    ...
    closePageFile(fh);
    fh = NULL; // Set fh to NULL after closing the file handle
...
PageNumber *getFrameContents(BM_BufferPool *const bm)
{
	...
	RC i;
	pages = (PageNumber(*)[(*bm).numPages])calloc((*bm).numPages, sizeof(PageNumber));
	pageInfo *temp;
	for (i = 0; i < (*bm).numPages; i++)
	{
...
bool *getDirtyFlags(BM_BufferPool *const bm)
{
	...
	RC i;
	isDirty = (bool(*)[(*bm).numPages])calloc((*bm).numPages, sizeof(bool));
	pageInfo *temp;
...
RC *getFixCounts(BM_BufferPool *const bm)
{
	...
	[(*bm).numPages];
	RC i;
	fixCounts = (RC(*)[(*bm).numPages])calloc((*bm).numPages, sizeof(RC));
	pageInfo *temp;
